...
bun test
bun test src/utils/__tests__/set.test.ts   # single file
bun test -t "difference"                   # by pattern

# Other
bun run check:unused   # detect unused exports (knip-bun)
bun run health         # health check (scripts/health-check.ts)
```
