...
export const init = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    // Best-effort OpenAI/Codex websocket startup prewarm. This is kept lazy so
    // non-OpenAI sessions do not pay the module load cost on startup.
    void import('../utils/openaiApiPrewarm.js')
      .then(({ scheduleOpenAIResponsesStartupPrewarm }) => {
        scheduleOpenAIResponsesStartupPrewarm()
      })
      .catch(() => {})
