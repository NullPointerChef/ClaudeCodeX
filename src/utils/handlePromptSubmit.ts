...
export async function handlePromptSubmit(
  ...
  if (
    ...
    // Clear UI state so "exit" doesn't linger as a pending user message.
    // setUserInputOnProcessing is set in onSubmit BEFORE handlePromptSubmit
    // is called — clear it here so the placeholder doesn't flash.
    onInputChange('')
    setCursorOffset(0)
    setPastedContents({})
    ...
    // .catch() ensures a silent rejection (module-load error, runtime
    // error in the /exit path) doesn't leave the process alive.
